...
public static class DbDataReaderMaybeExtensions
{
	public static async Task<Maybe<bool>> TryGetBooleanAsync(this DbDataReader reader, int ord,
		...
		await TryGetAsync(reader, ord, reader.GetBoolean, cancellationToken).ConfigureAwait(false);
	...
	{
		if (await reader.IsDBNullAsync(ord, cancellationToken).ConfigureAwait(false))
